...
const locales = {
  ...
  errWord: {
    'en': 'Unable to add to Shanbay notebook. This word is not in Shanbay\'s vocabulary database.',
...
export class Service extends SyncService<SyncConfig> {
  ...
  async addWord (text: string) {
    try {
      const url = 'http://www.shanbay.com/api/v1/bdc/search/?word=' + encodeURIComponent(text)
    ...
    try {
      var resLearning = await fetch('http://www.shanbay.com/api/v1/bdc/learning/', {
        'method': 'POST',
        'headers': {
  ...
  async isLogin (): Promise<boolean> {
    return Boolean(await browser.cookies.get({
