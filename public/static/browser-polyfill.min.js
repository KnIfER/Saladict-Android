...
var rover = window.chrome?chrome:{};

var isPlugin=rover.runtime!=undefined;


rover.isPlugin=isPlugin;

rover.isMobile=navigator.userAgent.indexOf('Android')>-1;

rover.closeStyle=function(e){
  var lastNode=document.currentScript.previousElementSibling;
  lastNode.disabled=!(browser.isMobile||document.documentElement.clientWidth<500);
}

rover.runtime_toggleFoldUnFold=function(e){
  var defP = document.getElementById('all');
  console.log(defP)
  var childNodes=defP.childNodes;
  for(var i=0;i<childNodes.length;i++) {
    childNodes[i].rct.toggleFolding(e);
  }
};

rover._Language=function(){
  return 'en'
};
			
var FuckListeners = [];
var FuckStorageListeners = [];
window.listeners=FuckListeners;
rover.runtime_onMessage_addListener=function(brw, e){
  //console.log('addListener!!!', brw, rover, chrome);
  if(isPlugin) {
    return brw.runtime.onMessage.addListener(e);
  } else {
    FuckListeners.push(e);
  }
};
rover.runtime_sendMessage=function(brw, e){
  console.log('runtime_sendMessage', e);
  if(isPlugin) {
    return brw.runtime.sendMessage(e);
  } else {
    var p = new Promise(function(res, reject){
      //if(0)
      for(var i=0;i<FuckListeners.length;i++) {
        var x = FuckListeners[i](e);
        console.log(x);
        if(x) {
          //return x;
          res(x)
      ...
      if(e.type===15){
        var fa=window.frames;
        for(var i=0;i<fa.length;i++) {
          var win=fa[i].window.FuckListeners;
          if(win) for(var i=0;i<win.length;i++) {
            var x = win[i](e);
            if(x) res(x);
          }
        }
...
rover.storage_get=function(brw, e, args){
	args=args[0];
	console.log('storage_get!!!', args.length, args);
  if(isPlugin) {
    var ret=brw.storage[e].get(args);
    console.log(brw.storage.sync.get(args));
    console.log(brw.storage.local.get(args));
    return ret;
  } else {
    var p = new Promise(function(res, reject){
      var items={};
      if(args instanceof Array) {
        for (var kii=0;kii<args.length;kii++) {
			    var kI=args[kii];
          var item = localStorage.getItem(kI);
          if(item) {
            items[kI]=JSON.parse(item);
          }
        }
        //console.log('————Array_stuf————', items);
      } else {
        var item = localStorage.getItem(args);
        if(item) {
          items[args]=JSON.parse(item);
        }
      }
      res(items);//JSON.stringify(item)
    });
    console.log(p);
    return p;
  }
};
rover.storage_set=function(brw, e, keys){
	console.log('storage_set storage_set storage_set!!!', keys);
  if(isPlugin) {
	  return brw.storage[e].set(keys);
  }else{
    var changes={}
	  for (var kI in keys) {
		  if(keys.hasOwnProperty(kI)){
        var cI={}
        cI['oldValue']=JSON.parse(localStorage.getItem(kI));
        cI['newValue']=keys[kI];
        localStorage.setItem(kI, JSON.stringify(keys[kI]));
        changes[kI]=cI;
        //console.log('——————', kI, JSON.stringify(keys[kI]));
      }
    }
    for(var i=0;i<FuckStorageListeners.length;i++) {
      var x = FuckStorageListeners[i](changes, e);
      console.log(x);
    }
...
rover.storage_onChanged_addListener=function(brw, e){
	console.log('storage_onChanged_addListener::', e);
  if(isPlugin) {
    return rover.storage.onChanged.addListener(e);
  }else{
	  FuckStorageListeners.push(e);
  }
};
rover.storage_onChanged_removeListener=function(brw, e){
	console.log('rmv!!!', brw, e, brw.storage);
  console.log('rmv!!!1', rover.storage);
  if(isPlugin) {
    return rover.storage.onChanged.removeListener(e);
  }else{
	  FuckStorageListeners.remove(e);
  }
};

rover._URL=function(e){
  if(isPlugin) {
    return rover.runtime.getURL(e);
  }else{
    if(e.charAt(0)=='/')
      e = e.substring(1,);
	  return e;
